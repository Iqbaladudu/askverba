...
export default function TestSidebarPage() {
  ...
  const { vocabulary, stats: vocabStats, loading: vocabLoading, error: vocabError } = useVocabulary({ limit: 10 })
      ...
      <div className="flex-1 p-6 space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            <p>This page shows the sidebar with real data and displays the raw data for comparison.</p>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent>
              <div className="space-y-2 text-sm">
                <p><strong>Loading:</strong> {vocabLoading ? 'Yes' : 'No'}</p>
                <p><strong>Error:</strong> {vocabError || 'None'}</p>
                <p><strong>Total Words:</strong> {vocabStats?.totalWords || 0}</p>
                <p><strong>Recent Words:</strong> {vocabulary?.length || 0}</p>
                {vocabulary && vocabulary.length > 0 && (
                  <div>
                    <p><strong>Latest Words:</strong></p>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2 text-sm">
                <p><strong>Loading:</strong> {progressLoading ? 'Yes' : 'No'}</p>
                <p><strong>Error:</strong> {progressError || 'None'}</p>
                <p><strong>Current Streak:</strong> {progress?.currentStreak || 0}</p>
                <p><strong>Average Accuracy:</strong> {progress?.averageAccuracy || 0}%</p>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2 text-sm">
                <p><strong>Loading:</strong> {practiceLoading ? 'Yes' : 'No'}</p>
                <p><strong>Error:</strong> {practiceError || 'None'}</p>
                <p><strong>Current Streak:</strong> {practiceStats?.currentStreak || 0}</p>
                <p><strong>Average Score:</strong> {practiceStats?.averageScore || 0}%</p>
                <p><strong>Total Practices:</strong> {practiceStats?.totalPractices || 0}</p>
              </div>
            </CardContent>
          </Card>
        </div>
